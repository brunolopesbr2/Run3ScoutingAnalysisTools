  ...
  :  displacedVertices_token_(consumes(params.getParameter<edm::InputTag>("displacedVertices")))
{
  ...
  produceWithRef<Run3ScoutingVertex, reco::Vertex>("DisplacedVertex");
...
void HLTScoutingRepackProducer::produce(edm::Event& iEvent, const edm::EventSetup& iSetup) {
  ...
  auto scoutingDisplacedVertex_ptr = std::unique_ptr<std::vector<Run3ScoutingVertex>>();
  ...
  putWithRef<Run3ScoutingVertex, reco::Vertex>(iEvent, "DisplacedVertex", recoDisplacedVertexRef_ptr, scoutingDisplacedVertex_ptr);
}
